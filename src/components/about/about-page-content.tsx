...
export function AboutPageContent({ variant = "page" }: AboutPageContentProps) {
  const Tag = variant === "section" ? "section" : "main";
  const sectionShellClass =
    variant === "section" ? "scroll-mt-24" : "min-h-screen pt-28";

  const sectionIntroSpacerClass =
    "border-t border-white/[0.12] pt-24 md:pt-32";

  const heroIntroClassName =
    "border-b border-zinc-200 pb-12 dark:border-white/10";
  ...
  return (
    <Tag
      id={variant === "section" ? "hakkimizda" : undefined}
      className={`bg-[#f7f7f4] text-zinc-900 transition-colors duration-300 dark:bg-zinc-950 dark:text-zinc-100 ${sectionShellClass}`}
    >
      {variant === "section" ? (
        <div aria-hidden="true" className={sectionIntroSpacerClass} />
      ) : null}
      <div className="mx-auto max-w-6xl px-6 pb-24 md:px-10">
        {variant === "section" ? (
          <div className={heroIntroClassName}>{heroIntro}</div>
        ) : (
          <Reveal className={heroIntroClassName} y={24}>
            {heroIntro}
          </Reveal>
        )}
        ...
        <div className="mt-14 grid grid-cols-1 gap-10 md:grid-cols-2">
          <Reveal
            className="rounded-3xl border border-zinc-200 bg-white p-6 dark:border-white/10 dark:bg-zinc-900"
            y={20}
          >
            <AboutTeamPortrait
            ...
            <p className="mt-4 text-sm font-light leading-relaxed text-zinc-700 dark:text-zinc-300">
              ...
              gözüyle harmanlayan Kevser, gelinliğinizin formundan saçınızın
              duruşuna kadar her detayı titizlikle yönetir; disiplinli yaklaşımını
              rahat ve eğlenceli bir enerjiyle dengeler.
            </p>
          </Reveal>
          ...
          >
            ...
            <p className="mt-4 text-sm font-light leading-relaxed text-zinc-700 dark:text-zinc-300">
              ...
              <span className="font-semibold lowercase">bekography</span> ile
              hazırlık sürecindeki disiplinli ve mükemmeliyetçi tavrını, çekim
              anında yüksek enerji ve samimiyete dönüştürerek{" "}
              <strong>güçlü bir deneyim</strong> sunar.
            </p>
          </Reveal>
        </div>
        ...
        <Reveal
          className="mt-14 rounded-3xl border border-zinc-200 bg-white p-7 dark:border-white/10 dark:bg-zinc-900 md:p-10"
          y={22}
        >
